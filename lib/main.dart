// ignore_for_file: prefer_const_constructors, prefer_const_literals_to_create_immutables, use_key_in_widget_constructors, library_private_types_in_public_api, unused_field, unused_local_variable
...
class AppColors {
  ...
  static const Color chatBubblePartner = Color(0xFF2C2C2E); // "Partner" is dark grey
}
...
class FirestoreService {
  ...
  Future<bool> acceptPairingRequest(String requestId, String requesterId) async {
    ...
    try {
      return await _firestore.runTransaction<bool>((transaction) async {
        ...
        transaction.update(_firestore.collection('users').doc(codeOwnerId), {'partnerId': requesterId, 'pairedAt': pairedAtTimestamp});
        transaction.update(_firestore.collection('users').doc(requesterId), {'partnerId': codeOwnerId, 'pairedAt': pairedAtTimestamp});
        transaction.update(requestDocRef, {'status': 'accepted', 'respondedAt': FieldValue.serverTimestamp()});
  ...
  Future<String?> uploadProfilePicture(XFile imageFile) async {
    ...
    try {
      ...
      final ref = _storage.ref().child('profile_pictures/$_currentUserUid/$fileName');
      ...
      builder: (context, snapshot) {
        ...
        if (streakCount > 0) {
          return Row(
            ...
            children: [
              Text('Heart to Heart', style: AppTextStyles.headline2.copyWith(fontSize: 20)),
              const SizedBox(width: 8),
          ...
          return Text('Heart to Heart', style: AppTextStyles.headline2.copyWith(fontSize: 20));
        }
      },
    );
  }
...
class _DashboardHomeTabState extends State<_DashboardHomeTab> with TickerProviderStateMixin {
  ...
  void _handleSend(String msg, String emj) {
    ...
    if (msg == "Your partner misses you") { // More specific check
  ...
  Widget _buildEmojiRow() {
    final emojis = [ {'e':'🥺','l':'Miss You','m':'Your partner is missing you'}, {'e':'❤️','l':'Love You','m':'Your partner loves you'}, {'e':'😢','l':'Sad','m':'Your partner is feeling sad'}, {'e':'🤩','l':'Excited','m':'Your partner is excited!'}, {'e':'💭','l':'Thinking','m':'Your partner is thinking of you'} ];
...
class _PartnerInfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return Card( // Show a loading/error state card
            child: Padding(
              padding: const EdgeInsets.all(16.0),
              child: Row(
                children: [
                  CircleAvatar(radius: 24, backgroundColor: AppColors.primaryBackground),
                  SizedBox(width: 12),
                  Text('Connecting to partner...', style: AppTextStyles.bodyText),
                ],
              ),
            ),
          );
        ...
        return Card(
          elevation: 0,
          color: AppColors.cardBackground.withOpacity(0.8), // Slightly transparent
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                // Streak
                if (streak > 0) ...[
                  Icon(Icons.local_fire_department_rounded, color: AppColors.streakFlame, size: 18),
                  SizedBox(width: 4),
                  Text(
                    '$streak',
                    style: AppTextStyles.bodyText.copyWith(fontSize: 16, color: AppColors.streakFlame, fontWeight: FontWeight.bold),
                  ),
                  SizedBox(width: 12),
                ],
                // Online Status
                ...
                Text(
                  status,
                  style: AppTextStyles.bodyText.copyWith(fontSize: 12, color: statusColor),
                ),
              ],
            ),
          ),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Flexible(
                child: Text(
                  ...
                  style: isMe ? AppTextStyles.chatBubbleMessage : AppTextStyles.chatBubbleMessage,
                ),
              ),
            ],
          ),
          ...
          Text(
            ...
            style: isMe ? AppTextStyles.chatBubbleTimestamp.copyWith(color: AppColors.textPrimary.withOpacity(0.7)) : AppTextStyles.chatBubbleTimestamp,
          ),
        ],
      ),
    );
...
class _DashboardProfileTabState extends State<_DashboardProfileTab> {
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      ...
      if (mounted) {
        // --- ⚠️ MOUNTED CHECK FIX ---
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to upload image.'), backgroundColor: AppColors.errorRed));
      }
    } finally {
      if (mounted) {
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        child: (photoUrl.isEmpty)
                            ?  Icon(Icons.person_rounded, size: 50, color: AppColors.textSecondary)
                            : null,
                      ),
                      if (_isEditing)
                        Positioned(
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          border: enabled ? OutlineInputBorder( borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.textSecondary) ) : InputBorder.none,
          enabledBorder: enabled ? OutlineInputBorder( borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.textSecondary) ) : InputBorder.none,
          focusedBorder: enabled ? OutlineInputBorder( borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.accentOrange, width: 2) ) : InputBorder.none,
