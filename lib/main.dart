// ignore_for_file: prefer_const_constructors, prefer_const_literals_to_create_immutables, use_key_in_widget_constructors, library_private_types_in_public_api, unused_field, unused_local_variable, use_build_context_synchronously
...
// --- ⚠️ NEW IMPORTS (for Profile Pictures) ---
import 'package:image_picker/image_picker.dart'; // For picking images
import 'dart:io'; // For File handling (Mobile)
...
class AppColors {
  ...
  static final Color textSecondary = Colors.white.withValues(
    alpha: 0.7,
  ); // Lighter white for secondary text
  ...
  static const Color bottomNavBackground =
      Colors.black; // Slightly transparent black

  // --- ⚠️ CHAT UI FIX: "Me" is Orange, "Partner" is Grey ---
  static const Color chatBubbleMe = Color(0xFFFF5200); // "Me" is now Orange
  static const Color chatBubblePartner = Color(
    0xFF2C2C2E,
  ); // "Partner" is dark grey
}
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        bottomNavigationBarTheme: BottomNavigationBarThemeData(
          ...
          selectedLabelStyle: AppTextStyles.smallButtonText.copyWith(
            fontSize: 12,
            fontFamily: 'Inter',
          ),
          unselectedLabelStyle: AppTextStyles.smallButtonText.copyWith(
            fontSize: 12,
            fontFamily: 'Inter',
          ),
      ...
      routes: {
        ...
        '/dashboard': (context) {
          ...
          if (partnerId == null) {
            ...
            return const Scaffold(
              body: Center(child: Text("Error: Missing partner info.")),
            );
          }
...
class FirebaseAuthService {
  ...
  Future<User?> signInWithGoogle() async {
    ...
    try {
      ...
      debugPrint("=" * 50);
      debugPrint(
        "If this is 'ApiException: 10' on Android, ensure your SHA-1 fingerprint is",
      );
      debugPrint(
        "correctly added to your Firebase project settings for the Android app.",
      );
      debugPrint(
        "Run './gradlew signingReport' in the 'android' folder to find your SHA-1 key.",
      );
      debugPrint("=" * 50);
      return null;
    }
  ...
  // --- ⚠️ FIX: Smarter User Doc creation ---
  ...
  Future<void> _createUserDocument(User user) async {
    ...
    if (!docSnapshot.exists) {
      ...
      final data = {
        ...
        'displayName':
            user.displayName ??
            user.email?.split('@')[0], // Ensure display name is set
        'createdAt': FieldValue.serverTimestamp(),
        'lastSentTimestamp': null,
        'currentStreak': 0,
        'partnerId': null,
        'pairedAt': null, // --- ⚠️ NEW: For "Connected Since" card ---
      ...
      await userDocRef.set(data); // Don't merge, just create
    } else {
      // --- EXISTING USER: Only update photo/name if they've changed ---
      // This is allowed by the rules (Rule 1)
      final existingData = docSnapshot.data()!;
      final Map<String, dynamic> updates = {};

      if (user.photoURL != null && user.photoURL != existingData['photoURL']) {
        updates['photoURL'] = user.photoURL;
      }
      if (user.displayName != null &&
          user.displayName != existingData['displayName']) {
        updates['displayName'] = user.displayName;
      }

      if (updates.isNotEmpty) {
        // This is an UPDATE, not a SET
...
class FirestoreService {
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>> getPartnerStream(
    String partnerId,
  ) {
  ...
  Future<String?> generatePairingCode(String userId) async {
    ...
    String code = (DateTime.now().microsecondsSinceEpoch % 1000000)
        .toString()
        .padLeft(6, '0');
  ...
  Future<String?> sendPairingRequest(String code, String currentUserId) async {
    ...
    final codeOwnerDoc = await _firestore
        .collection('users')
        .doc(codeOwnerId)
        .get();
    if (codeOwnerDoc.exists && codeOwnerDoc.data()?['partnerId'] != null) {
      debugPrint(
        "Cannot send request: Code owner ($codeOwnerId) already paired.",
      );
    ...
    try {
      final requestRef = await _firestore.collection('pairing_requests').add({
        ...
        'requesterName':
            ...
            _currentUserEmail ??
            'Unknown User', // Use display name or email
  ...
  getOutgoingPairingRequestStatus(String requestId) {
  ...
  Future<bool> acceptPairingRequest(
    String requestId,
    String requesterId,
  ) async {
    ...
    final requestDocRef = _firestore
        .collection('pairing_requests')
        .doc(requestId);
    final pairedAtTimestamp = FieldValue.serverTimestamp(); // --- ⚠️ NEW ---

    try {
      return await _firestore.runTransaction<bool>((transaction) async {
        // Re-fetch docs within transaction
        final freshRequestDoc = await transaction.get(requestDocRef);
        final freshCodeOwnerDoc = await transaction.get(
          _firestore.collection('users').doc(codeOwnerId),
        );
        final freshRequesterDoc = await transaction.get(
        ...
        if (freshCodeOwnerDoc.data()?['partnerId'] != null ||
            freshRequesterDoc.data()?['partnerId'] != null) {
          debugPrint("Transaction failed: One user is already paired.");
          transaction.update(requestDocRef, {
            'status': 'failed_already_paired',
            'respondedAt': FieldValue.serverTimestamp(),
          });
          return false;
        }

        // --- ⚠️ MODIFIED: Add pairedAt timestamp ---
        transaction.update(_firestore.collection('users').doc(codeOwnerId), {
          'partnerId': requesterId,
          'pairedAt': pairedAtTimestamp,
        });
        transaction.update(_firestore.collection('users').doc(requesterId), {
          'partnerId': codeOwnerId,
          'pairedAt': pairedAtTimestamp,
        });
        transaction.update(requestDocRef, {
          'status': 'accepted',
  ...
  Future<String?> uploadProfilePicture(XFile imageFile) async {
    ...
    try {
      ...
      final ref = _storage.ref().child(
        'profile_pictures/$_currentUserUid/$fileName',
      );
  ...
  Future<void> unpair(String partnerId) async {
    ...
    await _firestore
        .runTransaction((transaction) async {
          // --- ⚠️ MODIFIED: Also clear pairedAt date ---
          transaction.update(
            _firestore.collection('users').doc(_currentUserUid),
            {'partnerId': null, 'pairedAt': null},
          );
          transaction.update(_firestore.collection('users').doc(partnerId), {
            'partnerId': null,
            'pairedAt': null,
          });
        })
  ...
  int _calculateNewStreak(Timestamp? lastSent, int currentStreak) {
    ...
    if (lastSent != null) {
      ...
      final lastSentStartUtc = DateTime.utc(
        lastSentDate.year,
        lastSentDate.month,
        lastSentDate.day,
      );
...
class PairingCheckerScreen extends StatefulWidget {
  ...
  @override
  State<PairingCheckerScreen> createState() => _PairingCheckerScreenState();
}
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
            ...
            if (mounted) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Error loading data. Logging out.'),
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          debugPrint(
            "Pairing Check State: User document not found. Waiting or potential issue...",
          );
          return const Scaffold(
            body: Center(
              child: Column(
        ...
        final isDefaultName =
            ...
            (currentUserEmail != null &&
                displayName.contains(currentUserEmail.split('@')[0]));
        final isGoogleUser =
        ...
        debugPrint(
          "Pairing Check State: partnerId=$partnerId, displayName=$displayName, hasName=$hasName",
        );
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) {
            debugPrint(
              "[Nav Callback] Widget not mounted. Skipping navigation.",
            );
            return;
          }
          ...
          debugPrint(
            "[Nav Callback] Current=$currentRouteName, Target=$targetRouteName, Dialog=$isDialogShowing, LastNav=$_lastNavigatedRoute",
          );
          ...
          } else if (targetRouteName == 'SetDisplayNameScreen' &&
              currentRouteName != 'SetDisplayNameScreen') {
            needsNavigation = true;
          } else if (targetRouteName == 'PairingScreen' &&
              currentRouteName != 'PairingScreen') {
            needsNavigation = true;
          }

          // --- UNPAIR FIX: If we ARE on the dashboard but SHOULD be on the pairing screen, navigate ---
          if (targetRouteName == 'PairingScreen' &&
              currentRouteName == '/dashboard') {
            debugPrint(
              "[Nav Callback] Unpair detected! Navigating from Dashboard to PairingScreen.",
            );
            needsNavigation = true;
          }
          // --- End UNPAIR FIX ---
          ...
          if (targetRouteName == _lastNavigatedRoute && needsNavigation) {
            debugPrint(
              "[Nav Callback] Target route matches last navigated route. Skipping redundant navigation.",
            );
          ...
          if (needsNavigation) {
            ...
            _lastNavigatedRoute =
                targetRouteName; // Store target before navigating
            ...
            if (targetRouteName == '/dashboard') {
              ...
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: targetRouteBuilder,
                  settings: RouteSettings(name: targetRouteName),
                ),
              );
            } else {
              debugPrint(
                "[Nav Callback] Error: Target route builder was null.",
              );
        ...
        debugPrint(
          "Pairing Check: Build complete, waiting for navigation callback.",
        );
        return const Scaffold(
          body: Center(
            child: CircularProgressIndicator(color: AppColors.accentOrange),
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  // --- ⚠️ NEW: Password visibility toggle ---
    ...
    if (user == null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isSignUpMode
                ? 'Sign up failed. Check details or email might be in use.'
  ...
  Future<void> _handleGoogleSignIn(BuildContext context) async {
    ...
    if (user == null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Google Sign-In failed. Check console/setup (SHA-1/Client ID).',
          ),
          backgroundColor: AppColors.errorRed,
        ),
      );
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                // --- ⚠️ MODIFIED: Added Password Toggle ---
                TextField(
...
class _PairingScreenState extends State<PairingScreen>
  ...
  void initState() {
    ...
    if (_currentUser != null) {
      ...
      _userStreamSub = _firestoreService.userStream.listen((userDoc) {
        ...
        if (partnerId == null) {
          // --- ⚠️ RE-PAIRING BUG FIX: Check if we are already generating/have a code ---
          if (!_isUserDocReady ||
              (_generatedCode == null && !_isGeneratingCode)) {
            debugPrint(
              "[PairingScreen] User confirmed unpaired. Generating code.",
            );
            _generateNewCode();
          }
          setState(() => _isUserDocReady = true); // Mark as ready *after* check
        } else {
  ...
  Future<void> _generateNewCode() async {
    ...
    setState(() {
      ...
      _generatedCode = null;
      _alreadyPairedError = false;
    });

    try {
      // We call the service, which *will* re-check the user doc
      String? code = await _firestoreService.generatePairingCode(
        _currentUser!.uid,
      );
      if (mounted) {
        setState(() {
          ...
          _alreadyPairedError =
              (code ==
              null); // This is the case where the service re-check fails
        });
      }
    } catch (e) {
  ...
  void _listenForIncomingRequests() {
    ...
    _incomingRequestSub = _firestoreService.getIncomingPairingRequests().listen(
      (snapshot) {
        ...
        debugPrint(
          "[Listener] Found ${snapshot.docs.length} pending requests in snapshot.",
        );
  ...
  Future<void> _showPairingRequestDialog(
    ...
    debugPrint(
      "[Dialog] Checking if route is current: $isCurrent, Is Popup: $isPopup",
    );
    if (!isCurrent || isPopup) {
      debugPrint(
        "[Dialog] Skipping dialog, route is not current or a popup is already showing.",
      );
      return;
    }

    debugPrint(
      "[Dialog] Displaying confirmation dialog for request $requestId...",
    );
    final confirmed =
        await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(
              '$requesterName wants to pair with you. Accept?',
              style: AppTextStyles.bodyText,
            ),
            actionsPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
            actions: [
    ...
    if (confirmed) {
      ...
      bool success = await _firestoreService.acceptPairingRequest(
        requestId,
        requesterId,
      );
      if (mounted && !success) {
        // Show error only if accept failed
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Failed to accept request. Maybe one user got paired?',
              ),
              backgroundColor: AppColors.errorRed,
            ),
          );
  ...
  Future<void> _sendPairingRequest() async {
    ...
    try {
      String? result = await _firestoreService.sendPairingRequest(
        code,
        _currentUser!.uid,
      );
      ...
      if (result == "failed_already_paired") {
        setState(() {
          _pairingStatus = PairingStatus.failedAlreadyPaired;
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('You or the other user is already paired.'),
              backgroundColor: AppColors.errorRed,
            ),
          );
        }
        ...
        // result is null - invalid code or other error
        setState(() {
          _pairingStatus = PairingStatus.failedError;
          _isSendingRequest = false;
        });
        // --- ⚠️ MOUNTED CHECK FIX ---
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Invalid code or failed to send request.'),
              backgroundColor: AppColors.errorRed,
            ),
          );
        }
      ...
      if (mounted) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Failed to send pairing request.'),
              backgroundColor: AppColors.errorRed,
            ),
          );
        }
  ...
  void _listenToOutgoingRequest(String requestId) {
    _outgoingRequestSub = _firestoreService
        .getOutgoingPairingRequestStatus(requestId)
        .listen(
          (snapshot) {
            if (!mounted) return;
            if (!snapshot.exists) {
              ...
              if (_pairingStatus == PairingStatus.requestSent) {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Pairing request expired or failed.'),
                      backgroundColor: AppColors.errorRed,
                    ),
                  );
                }
            ...
            switch (status) {
              ...
              case 'declined':
                setState(() {
                  _pairingStatus = PairingStatus.declined;
                  _isSendingRequest = false;
                });
                // --- ⚠️ MOUNTED CHECK FIX ---
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Partner declined the request.'),
                      backgroundColor: AppColors.errorRed,
                    ),
                  );
                }
              ...
              case 'failed_already_paired':
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Pairing failed: One user was already paired.',
                      ),
                      backgroundColor: AppColors.errorRed,
                    ),
                  );
                ...
                if (_pairingStatus == PairingStatus.requestSent) {
                  ...
                  if (status != 'cancelled_by_requester') {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Pairing failed unexpectedly.'),
                          backgroundColor: AppColors.errorRed,
                        ),
                      );
                    }
          ...
          onError: (error) {
            debugPrint(
              "Error listening to outgoing request $requestId: $error",
            );
            if (mounted && _pairingStatus == PairingStatus.requestSent) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Error checking pairing status.'),
                    backgroundColor: AppColors.errorRed,
                  ),
                );
              }
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Error receiving pairing requests. Check Firestore index & console.',
              ),
              backgroundColor: AppColors.errorRed,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (!_alreadyPairedError &&
                _isUserDocReady) // --- ⚠️ NEW: Hide button until ready
              TextButton.icon(
                icon: const Icon(Icons.refresh_rounded, size: 18),
                label: const Text('Generate New Code'),
                onPressed: _isGeneratingCode
                    ? null
                    : _generateNewCode, // Disable while generating
              ),
          ],
        ),
      ),
  ...
  Widget _buildCodeErrorDisplay() {
    return Padding(
      ...
      child: Column(
        // Use Column for text and retry button
        children: [
          Text(
            'Failed to generate code.\nPlease check connection.',
            textAlign: TextAlign.center,
            style: AppTextStyles.bodyText.copyWith(color: AppColors.errorRed),
          ),
        ],
      ),
    );
          ...
          children: [
            ...
            if (_pairingStatus != PairingStatus.requestSent)
              TextField(
                ...
                decoration: InputDecoration(
                  hintText:
                      '_ _ _ _ _ _', // --- ⚠️ NEW: Hint text with spaces ---
                  hintStyle: AppTextStyles.headline1.copyWith(
                    ...
                    fontSize: 32,
                    letterSpacing:
                        10, // --- ⚠️ NEW: Add letter spacing to hint ---
                  ),
                ),
            ...
            _isSendingRequest
                ? (_pairingStatus == PairingStatus.requestSent
                      ...
                      : const CircularProgressIndicator(
                          color: AppColors.accentOrange,
                        ) // Show spinner while initially sending
                        )
                : ElevatedButton.icon(
                    // Show Connect button
  ...
  Future<void> _fetchPartnerId() async {
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error getting partner data. Try restarting.'),
            backgroundColor: AppColors.errorRed,
          ),
        );
      }
  ...
  Future<void> _saveName() async {
    ...
    if (_partnerId == null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error finding partner data. Please restart.'),
            backgroundColor: AppColors.errorRed,
          ),
        );
      }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to save name. Check connection/permissions.'),
            backgroundColor: AppColors.errorRed,
          ),
        );
        setState(() {
              ...
              child: Column(
                ...
                children: [
                  ...
                  _isLoading
                      ...
                      : ElevatedButton(
                          onPressed: _partnerId == null ? null : _saveName,
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void initState() {
    ...
    _notificationSub = _firestoreService.getNotificationsStream().listen(
      _handleNotificationSnapshot,
      onError: (error) {
        debugPrint("Vibration Notification Stream Error: $error");
      },
    );
    ...
    _selfUserStreamSub = _firestoreService.userStream.listen(
      (snapshot) {
        // --- ⚠️ MOUNTED CHECK FIX ---
        if (!mounted) return;
        ...
        if (snapshot.exists && snapshot.data()?['partnerId'] == null) {
          ...
          debugPrint(
            "Dashboard listener: partnerId is null. Navigating to /checker.",
          );
          if (mounted) {
            // Use rootNavigator to clear the dashboard stack and force re-check
            Navigator.of(context, rootNavigator: true).pushAndRemoveUntil(
              MaterialPageRoute(
                builder: (_) => const PairingCheckerScreen(),
                settings: const RouteSettings(name: '/checker'),
              ),
              (route) => false,
            );
          }
  ...
  Future<void> _handleNotificationSnapshot(QuerySnapshot snapshot) async {
    debugPrint(
      "Vibration: Snapshot received with ${snapshot.docChanges.length} changes.",
    ); // --- DEBUG LOG ---
    if (_isFirstNotificationLoad) {
      debugPrint(
        "Vibration: First load, skipping vibration.",
      ); // --- DEBUG LOG ---
        ...
        try {
          ...
          if (senderId != null &&
            ...
            if (timestamp != null) {
              ...
              if (age.inSeconds < 10) {
                // Only vibrate for very new messages from partner
                debugPrint(
                  "Vibration: Message is new, VIBRATING.",
                ); // --- DEBUG LOG ---
                _vibrate();
              } else {
                debugPrint(
                  "Vibration: Message is old (${age.inSeconds}s), skipping.",
                ); // --- DEBUG LOG ---
              }
            }
          } else {
            debugPrint(
              "Vibration: Sender is current user, skipping.",
            ); // --- DEBUG LOG ---
          }
        } catch (e) {
  ...
  Future<void> _vibrate() async {
    try {
      ...
      if (hasVibrator == true) {
        debugPrint(
          "Vibration: Device has vibrator, calling vibrate().",
        ); // --- DEBUG LOG ---
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        decoration: BoxDecoration(
          color: AppColors.bottomNavBackground.withValues(alpha: 0.85),
        ),
        child: ClipRect(
  ...
  Widget _buildAppBarTitle() {
    ...
    // Default: Home Tab Title (No Partner Card here)
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: _firestoreService.userStream,
      builder: (context, snapshot) {
        if (!snapshot.hasData || !snapshot.data!.exists) {
        ...
        final streakCount =
            snapshot.data?.data()?['currentStreak'] as int? ?? 0;
        if (streakCount > 0) {
          return Row(
            mainAxisAlignment: MainAxisAlignment.center,
...
class _DashboardHomeTabState extends State<_DashboardHomeTab>
  ...
  // This function is now simplified, no TapDetails needed
  void _handleSend(String msg, String emj) {
    ...
    if (msg == "Your partner misses you") {
      // More specific check
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Sent "$msg" $emj', style: AppTextStyles.bodyText),
          backgroundColor: AppColors.cardBackground,
              ...
              return SingleChildScrollView(
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      // --- ⚠️ NEW: Partner Info Card ---
                      SizedBox(height: 16),
                      _PartnerInfoCard(partnerId: widget.partnerId),

                      Padding(
                        padding: EdgeInsets.symmetric(
                          vertical: max(32.0, cons.maxHeight * 0.1),
                        ),
                      ...
                      const SizedBox(
                        height: 100,
                      ), // --- FIX: Increased space for glassy nav bar
                    ],
                  ),
                ),
              );
  ...
  Widget _buildMainButton(BoxConstraints c) {
    ...
    return GestureDetector(
      ...
      // --- FIX: Use simplified tap handler ---
      onTapUp: (d) => _handleSend("Your partner misses you", "❤️"),
      ...
      child: AnimatedScale(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: AppColors.accentOrange.withAlpha(102),
                blurRadius: 30,
                spreadRadius: 8,
              ),
            ],
  ...
  Widget _buildEmojiRow() {
    final emojis = [
      {'e': '🥺', 'l': 'Miss You', 'm': 'Your partner is missing you'},
      {'e': '❤️', 'l': 'Love You', 'm': 'Your partner loves you'},
      {'e': '😢', 'l': 'Sad', 'm': 'Your partner is feeling sad'},
      {'e': '🤩', 'l': 'Excited', 'm': 'Your partner is excited!'},
      {'e': '💭', 'l': 'Thinking', 'm': 'Your partner is thinking of you'},
    ];
    return Wrap(
...
// --- ⚠️ NEW: Partner Info Card Widget ---
class _PartnerInfoCard extends StatelessWidget {
  final String partnerId;
  const _PartnerInfoCard({required this.partnerId});
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: firestoreService.getPartnerStream(partnerId),
      builder: (context, snapshot) {
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return Card(
            // Show a loading/error state card
            child: Padding(
        ...
        final partnerName = partnerData['displayName'] ?? 'Your Partner';
        final partnerPhotoUrl = partnerData['photoURL'] as String?;
        final isOnline = partnerData['isOnline'] as bool? ?? false;
        final lastSeen = partnerData['lastSeen'] as Timestamp?;
        final streak = partnerData['currentStreak'] as int? ?? 0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var particle in particles) {
      ...
      final currentOpacity =
          particle.opacity *
          (1.0 - animation.value); // Fade out with animation value
      if (currentOpacity <= 0) continue; // Skip if faded out

      paint.color = AppColors.accentOrange.withValues(alpha: currentOpacity);
      ...
      path.cubicTo(
        ...
        particle.y - particle.size * 0.7, // Top left curve
      ...
      path.cubicTo(
        particle.x - particle.size * 0.8,
        particle.y - particle.size * 0.3, // Top right curve back to start
        particle.x - particle.size * 0.4,
        particle.y - particle.size * 0.7,
        particle.x,
        particle.y - particle.size * 0.3,
      );
...
class _HeartParticle {
  ...
  _HeartParticle({
    ...
    required this.screenHeight,
           ...
           40, // Start near horizontal center
       y =
           screenHeight * 0.5 +
           random.nextDouble() * 50 -
           25, // Start near vertical center
       size = random.nextDouble() * 15 + 10,
       opacity =
           random.nextDouble() * 0.3 +
           0.7, // Start slightly less than full opacity (0.7-1.0)
       speedY = random.nextDouble() * -3 - 2, // Move UPWARDS
       speedX =
           random.nextDouble() * 2 - 1; // Slight horizontal drift (-1 to 1)
  ...
  bool update() {
    ...
    return opacity > 0 &&
        y > -size; // Check if still visible or about to become visible
  }
}
...
class _DashboardHistoryTab extends StatelessWidget {
  ...
  _DashboardHistoryTab({required this.partnerId})
    : partnerStream = FirestoreService().getPartnerStream(
        partnerId,
      ), // Init partner stream
  ...
  String _formatTimestamp(Timestamp? t) {
    ...
    if (difference.inMinutes < 60) return "${difference.inMinutes}m ago";
    if (difference.inHours < 24 && now.day == dt.day) {
      return DateFormat('h:mm a').format(dt);
    }
    if (difference.inDays < 2 && now.day == dt.day + 1) {
      return "Yesterday"; // Simpler
    }
              ...
              Expanded(
                ...
                child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, userSnapshot) {
                    final myName =
                        userSnapshot.data?.data()?['displayName'] ?? 'Me';
                    final myPhotoUrl =
                    ...
                    >(
                      ...
                      builder: (context, partnerSnapshot) {
                        final partnerName =
                            partnerSnapshot.data?.data()?['displayName'] ??
                            'Partner';
                        final partnerPhotoUrl =
                            partnerSnapshot.data?.data()?['photoURL']
                                as String?;

                        return _buildNotificationList(
                          context:
                              context, // --- ⚠️ CHAT UI FIX: Pass context ---
                          myName: myName,
  ...
  Widget _buildNotificationList({
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (c, s) {
        ...
        if (s.hasError) {
          debugPrint("Notification List Error: ${s.error}");
          return const Center(
            child: Text(
        ...
        return ListView.builder(
          ...
          itemBuilder: (c, i) {
            ...
            return _buildChatBubble(
              ...
              showAvatar:
                  !isSameSenderAsPrevious, // Only show avatar if sender changed
            );
          },
        );
      },
  ...
  Widget _buildChatBubble({
    ...
    final bubble = Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Flexible(
                child: Text(
                  message,
                  // --- ⚠️ CHAT UI FIX: "Me" text is primary (white) ---
                  style: isMe
                      ? AppTextStyles.chatBubbleMessage
                      : AppTextStyles.chatBubbleMessage,
                ),
              ),
            ],
          ...
          Text(
            timestamp,
            // --- ⚠️ CHAT UI FIX: "Me" timestamp is light ---
            style: isMe
                ? AppTextStyles.chatBubbleTimestamp.copyWith(
                    color: AppColors.textPrimary.withValues(alpha: 0.7),
                  )
                : AppTextStyles.chatBubbleTimestamp,
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        ...
        crossAxisAlignment:
            CrossAxisAlignment.end, // Align avatar to bottom of bubble
        children: [
          // --- PARTNER (LEFT) ---
          if (!isMe) ...[avatar, SizedBox(width: 8)],
...
class _DashboardProfileTabState extends State<_DashboardProfileTab> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      final userDoc = await _firestoreService.userStream.firstWhere(
        (doc) => doc.exists,
      );
      if (mounted) {
      ...
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Failed to update profile. Check Rules/Connection.',
              ),
              backgroundColor: AppColors.errorRed,
            ),
          );
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      ...
      final downloadUrl = await _firestoreService.uploadProfilePicture(image);
      ...
      if (downloadUrl != null) {
        ...
        throw Exception('Upload failed, download URL was null.');
      }
    } catch (e) {
  ...
  Future<void> _handleUnpair() async {
    if (widget.partnerId == null || !mounted || _isLoading) return;
    final confirmed = await showConfirmationDialog(
      context,
      title: 'Unpair Partner?',
      content:
          'Are you sure? This will remove your connection, but you can pair again later.',
      confirmText: 'Unpair',
      isDestructive: true,
    );
    if (confirmed && mounted) {
      try {
        ...
        debugPrint(
          "Unpair successful. Auth stream will handle navigation.",
        ); // Debugging
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Unpaired successfully.'),
              backgroundColor: Colors.green,
            ),
          );
          // Navigation is handled by PairingCheckerScreen stream
  ...
  Future<void> _handleLogout() async {
    ...
    if (confirmed) {
      try {
        ...
        debugPrint(
          "Logout successful. Auth stream should handle navigation.",
        ); // Debugging
  ...
  Widget build(BuildContext context) {
    final user = FirebaseAuth.instance.currentUser;
    final String photoUrl = _userData['photoURL'] ?? user?.photoURL ?? '';
    final Timestamp? pairedAt = _userData['pairedAt'] as Timestamp?;

    return GradientBackground(
      child: ResponsiveCenter(
        child: SingleChildScrollView(
          // Essential for keyboard and content overflow
          child: Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(
              children: [
                ...
                if (widget.partnerId != null && pairedAt != null)
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(16.0),
                      child: Row(
                        ...
                        children: [
                          ...
                          Flexible(
                            child: Text(
                              'Connected since ${DateFormat('MMMM d, yyyy').format(pairedAt.toDate())}',
                              style: AppTextStyles.bodyText.copyWith(
                                fontWeight: FontWeight.w600,
                              ),
                            ),
                ...
                const SizedBox(
                  height: 100,
                ), // --- FIX: Increased bottom padding for nav bar on scroll
              ],
            ),
          ),
        ),
    ...
    return Padding(
      ...
      child: TextField(
        ...
        textInputAction: isLast
            ...
            : TextInputAction.next, // Use 'done' for the last field
        onSubmitted: isLast
            ? (_) => _toggleEditSave()
            : null, // Trigger save on 'done' for last field
        decoration: InputDecoration(
          labelText: label,
          ...
          fillColor: enabled
              ...
              : AppColors.primaryBackground.withValues(
                  alpha: 0.5,
                ), // Make view mode slightly different
          border: enabled
              ? OutlineInputBorder(
                  borderRadius: BorderRadius.circular(12),
